...
import elementaireHero from "@/assets/elementaire-hero.jpg";

export const Route = createFileRoute("/elementaire")({
  head: () => ({
    meta: [
      { title: "Uniformes Élémentaire — Saint-Jacques de Compostelle" },
      { name: "description", content: "Sélection d'uniformes pour les élèves d'élémentaire (CP → CM1)." },
    ],
  }),
  component: ElementairePage,
});

const sizes = ["5 ans", "6 ans", "7 ans", "8 ans", "10 ans", "12 ans"];
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border bg-card">
        <div className="mx-auto flex max-w-7xl items-center gap-1.5 px-4 py-3 text-xs text-muted-foreground sm:px-6 lg:px-8">
          ...
          <span className="text-foreground">Élémentaire (CP → CM1)</span>
        </div>
      </div>
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[1.1fr_1fr] lg:px-8">
          <div>
            ...
            <p className="mt-3 max-w-xl leading-relaxed text-muted-foreground">
              Tenue officielle pour les classes de CP, CE1, CE2 et CM1. Portée au quotidien par tous les élèves.
            </p>
            <div className="mt-6 flex flex-wrap gap-2 text-xs">
              <span className="rounded-full border border-border bg-card px-3 py-1.5 text-muted-foreground">1 produit</span>
              <span className="rounded-full border border-border bg-card px-3 py-1.5 text-muted-foreground">Tailles 5 → 12 ans</span>
          ...
          <div className="relative h-64 overflow-hidden rounded-3xl border border-border lg:h-80">
            <img src={elementaireHero} alt="Élèves d'élémentaire en blouse" className="h-full w-full object-cover" loading="lazy" />
...
function ProductCard({ product }: { product: (typeof products)[number] }) {
  ...
  const [size, setSize] = useState("8 ans");
    ...
    <article className="group overflow-hidden rounded-3xl border border-border bg-card transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-card)]">
      <Link to={product.href} className="block">
        <div className="relative aspect-square overflow-hidden" style={{ backgroundColor: "#f3edE0" }}>
          <img src={product.image} alt={product.name} className="h-full w-full object-contain p-4" loading="lazy" />
      ...
      <div className="p-6">
        ...
        <div className="mt-4">
          ...
          <div className="mt-2"><ChildPicker value={childId} onChange={setChildId} filter={(c) => c.section === "Élémentaire"} /></div>
        </div>
            ...
            {sizes.map((s) => (
              <button key={s} onClick={() => setSize(s)}
                className={`h-9 px-2 min-w-[3.5rem] rounded-md border text-xs font-medium transition-all ${
                  size === s ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-foreground hover:border-primary/40"
                }`}>
                {s}
              </button>
            ))}
