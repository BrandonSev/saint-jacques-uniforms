...
interface Props { prenom?: string; orderNumber?: string; items?: Item[]; total?: number }

function OrderConfirmationEmail({ prenom = '', orderNumber = '', items = [], total = 0 }: Props) {
  return (
    <EmailLayout preview={`Commande ${orderNumber} confirmée`} title={`Commande ${orderNumber} confirmée`}>
      ...
      <table width="100%" cellPadding={0} cellSpacing={0} style={{ borderCollapse: 'collapse', margin: '16px 0', fontSize: 14 }}>
        <thead>
          <tr>
            <th align="left" style={{ padding: '8px 0', borderBottom: '2px solid #0f3a5f' }}>Article</th>
            <th style={{ padding: '8px 0', borderBottom: '2px solid #0f3a5f' }}>Qté</th>
            <th align="right" style={{ padding: '8px 0', borderBottom: '2px solid #0f3a5f' }}>Total</th>
          </tr>
        </thead>
        <tbody>
          {items.map((i, idx) => (
      ...
      <Text style={muted}>Merci de votre confiance,<br />L'équipe Saint-Jacques</Text>
    </EmailLayout>
  )
}
...
export const template = {
  ...
  previewData: { prenom: 'Marie', orderNumber: 'CMD-20260504-C001-001', total: 120, items: [{ name: 'Polo bleu', size: '12 ans', qty: 2, price: 25, child: 'Léa' }, { name: 'Pull marine', size: 'M', qty: 1, price: 70, child: 'Tom' }] },
