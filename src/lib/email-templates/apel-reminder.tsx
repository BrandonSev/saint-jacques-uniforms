...
function ApelReminder({ prenom, familyName, deadline = '30 juin 2026', appUrl = APP_URL, customMessage }: Props) {
  return (
    ...
    >
      ...
      <Text style={text}>
        L'<strong>Association des Parents d'Élèves</strong> de Saint-Jacques-de-Compostelle
        vous rappelle que vous n'avez pas encore passé commande des uniformes pour la rentrée 2026.
      </Text>
      {customMessage && <Text style={text}>{customMessage}</Text>}
      <Text style={text}>
        Pour garantir la fabrication et la livraison à temps, merci de passer votre commande
        avant le <strong>{deadline}</strong>. Au-delà de cette date, nous ne pourrons plus garantir
        la disponibilité des blouses pour la rentrée.
      </Text>
      <Button href={`${appUrl}/boutique`} style={button}>Commander maintenant</Button>
      <Text style={muted}>
        Cet email vous est envoyé par l'APEL via la boutique Saint-Jacques.<br />
